    ...
    fs_variant{std::move(fs_variant)}
{
	...
	std::vector<uint32_t> temp_input_attachments{};
	for (const auto &it : this->input_attachments)
	{
        temp_input_attachments.push_back(it.second);
	}
	set_input_attachments(temp_input_attachments);
}
...
PostProcessingSubpass &PostProcessingSubpass::bind_storage_image(const std::string &name, core::SampledImage &&new_image)
{
...
PostProcessingSubpass &PostProcessingSubpass::bind_storage_buffer(const std::string &name, const BufferAllocation &new_alloc)
{
	auto it = storage_buffers.find(name);
	...
	{
		storage_buffers.emplace(name, &new_alloc);
	}
...
void PostProcessingSubpass::draw(CommandBuffer &command_buffer)
{
    ...
    // Bind storage buffers to set = 0, binding = <according to name>
    for (const auto &it : storage_buffers)
    {
        if (auto layout_binding = bindings.get_layout_binding(it.first))
        {
            auto bufAlloc = it.second;
            auto &buf = const_cast<vkb::BufferAllocation*>(bufAlloc)->get_buffer();
            command_buffer.bind_buffer(buf, bufAlloc->get_offset(), bufAlloc->get_size(), 0, layout_binding->binding, 0);
        }
    }
...
void PostProcessingRenderPass::transition_attachments(
    ...
    const SampledAttachmentSet &sampled_attachments,
    const AttachmentSet &       output_attachments,
    const SampledAttachmentSet &storage_attachments,
    ...
    RenderTarget &              fallback_render_target)
{
    ...
    for (const auto &storage : storage_attachments)
    {
        auto *storage_rt = storage.first ? storage.first : &render_target;
        const auto prev_layout = storage_rt->get_layout(storage.second);
        ...
        barrier.src_access_mask = prev_pass_barrier_info.image_write_access | prev_pass_barrier_info.image_read_access;
        barrier.dst_access_mask = VK_ACCESS_SHADER_WRITE_BIT;

        command_buffer.image_memory_barrier(storage_rt->get_views().at(storage.second), barrier);
        storage_rt->set_layout(storage.second, barrier.new_layout);
    }
...
void PostProcessingRenderPass::prepare_draw(CommandBuffer &command_buffer, RenderTarget &fallback_render_target)
{
	...
	SampledAttachmentSet sampled_attachments, storage_attachments;
	...
	for (auto &step_ptr : pipeline.get_subpasses())
	{
        ...
        for (auto &it : step.get_storage_images())
        {
            if (const uint32_t *storage_attachment = it.second.get_target_attachment())
            {
                auto *image_rt = it.second.get_render_target();
                storage_attachments.insert({image_rt, *storage_attachment});
            }
        }
	...
	transition_attachments(input_attachments, sampled_attachments, output_attachments, storage_attachments,
