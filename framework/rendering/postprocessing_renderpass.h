 ...
 */
using StorageImageMap = std::unordered_map<std::string, core::SampledImage>;

/**
 * @brief Maps in-shader binding names to the memory blocks to bind for storage buffers.
 */
using StorageBufferMap = std::unordered_map<std::string, const BufferAllocation *>;
...
class PostProcessingSubpass : public Subpass
{
  public:
	...
	*/
    ...
    /**
    * @brief Maps the names of storage buffers in the shader to vkb::BufferAllocation.
    *        These are given as buffers to the subpass, at set 0; they are bound automatically according to their name.
    */
    inline const StorageBufferMap &get_storage_buffers() const
    {
	...
	*/
	 ...
	 */
	PostProcessingSubpass &bind_storage_image(const std::string &name, core::SampledImage &&new_image);

	/**
	 * @brief Changes (or adds) the buffer allocation at name for this step.
	 */
	PostProcessingSubpass &bind_storage_buffer(const std::string &name, const BufferAllocation &new_alloc);
  ...
  private:
    ...
    StorageBufferMap storage_buffers{};
