...
PostProcessingComputePass &PostProcessingComputePass::bind_storage_buffer(const std::string &name, const BufferAllocation &new_alloc)
{
...
void PostProcessingComputePass::transition_images(CommandBuffer &command_buffer, RenderTarget &default_render_target)
{
	...
	for (const auto &storage : storage_images)
	{
		if (const uint32_t *attachment = storage.second.get_target_attachment())
		{
			...
			const bool writable = !(resource->qualifiers & ShaderResourceQualifiers::NonWritable);
			...
			if (storage_rt->get_layout(*attachment) == barrier.new_layout && barrier.new_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
			{
				// No-op
				continue;
			}
...
void PostProcessingComputePass::draw(CommandBuffer &command_buffer, RenderTarget &default_render_target)
{
	...
	// Dispatch compute
	if (indirectBuffer)
		command_buffer.dispatch_indirect(const_cast<vkb::BufferAllocation*>(indirectBuffer)->get_buffer(), indirectBuffer->get_offset());
	else
